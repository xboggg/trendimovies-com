...
On 2026-08-25, the root cause behind step 5's "known tracked documentation deletions" was found and fixed: `/usr/local/bin/trendimovies-install`'s rsync had no exclusion for `AGENTS.md`, `DOCS/`, or `.github/`, so any code deploy (not just a doc change) silently deleted them from the live checkout, since they are also intentionally absent from the deploy tarball itself. Nothing was ever lost from git/GitHub -- only the live working tree's copy was affected. The rsync now excludes those three paths (matching the existing `*.bak*` / `public/images/posters/` exclusions added the same way for a prior, related bug). Step 5's manual restore should no longer be routinely necessary going forward; it remains documented here as a safety net.
