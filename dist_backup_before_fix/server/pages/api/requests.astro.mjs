...
const POST = async ({ request, clientAddress }) => {
  try {
    ...
    const recentCheck = await fetch(
      `${POSTGREST_URL}/content_requests?ip_address=eq.${ip}&created_at=gte.${new Date(Date.now() - 5 * 60 * 1e3).toISOString()}&select=count`,
    ...
    const requestData = {
      ...
      notes: notes?.trim().substring(0, 1e3) || null,
      status: "pending",
