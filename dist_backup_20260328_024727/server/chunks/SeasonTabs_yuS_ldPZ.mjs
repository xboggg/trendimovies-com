import { s as sanitize_props, a as spread_props, b as slot, i as fallback, d as ensure_array_like, e as escape_html, f as attr_class, c as attr, h as stringify, j as bind_props } from './_@astro-renderers_DbfXOWuU.mjs';
/* empty css                          */
/* empty css                          */
...
import { C as Chevron_down } from './BaseLayout_BH5Mr41D.mjs';
...
function SeasonTabs($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div><div class="flex items-center gap-3 mb-6"><label for="season-select" class="text-sm font-semibold whitespace-nowrap" style="color: var(--text-secondary);">Season:</label> <div class="relative flex-1 max-w-xs">`);
		...
		$$renderer.select(
			...
			($$renderer) => {
				...
				for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
					...
					$$renderer.option(
						{ value: season.season_number, class: '' },
						($$renderer) => {
							$$renderer.push(`${escape_html(season.name || `Season ${season.season_number}`)}
            ${escape_html(season.episode_count ? ` (${season.episode_count} eps)` : '')}`);
						},
						'svelte-3dl70z'
					);
				}
		...
		$$renderer.push(` <div class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2" style="color: var(--text-secondary);"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></div></div> `);

		if (currentEpisodes.length > 0) {
			$$renderer.push('<!--[-->');
			$$renderer.push(`<span class="text-xs px-3 py-1 rounded-full font-medium" style="background: var(--bg-card); color: var(--text-secondary); border: 1px solid var(--border);">${escape_html(currentEpisodes.length)} Episode${escape_html(currentEpisodes.length !== 1 ? 's' : '')}</span>`);
		} else {
			$$renderer.push('<!--[!-->');
		}
		...
		for (let $$index_2 = 0, $$length = each_array_1.length; $$index_2 < $$length; $$index_2++) {
			...
			if (hasDownloads && expandedDownloads === episode.id) {
				...
				for (let $$index_1 = 0, $$length = each_array_2.length; $$index_1 < $$length; $$index_1++) {
					...
					$$renderer.push(`<button${attr_class(`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors ${stringify(getQualityColor(link.quality))}`, 'svelte-3dl70z')}>`);
