...
const STREAMING_SERVERS = {
  ...
  server3: {
    id: "server3",
    name: "Server 3",
    getMovieUrl: (tmdbId) => `https://www.vidbinge.to/movie/${tmdbId}`,
    getTVUrl: (tmdbId, season, episode) => `https://www.vidbinge.to/tv/${tmdbId}/${season}/${episode}`,
    idType: "tmdb"
  },
  server4: {
    id: "server4",
    name: "Server 4",
    getMovieUrl: (tmdbId) => `https://vidfast.pro/movie/${tmdbId}`,
    getTVUrl: (tmdbId, season, episode) => `https://vidfast.pro/tv/${tmdbId}/${season}/${episode}`,
    idType: "tmdb"
  }
};
...
function VideoPlayer($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		let releaseDate = fallback($$props['releaseDate'], null);
		...
		if (isUnreleased) {
			$$renderer.push('<!--[-->');
			$$renderer.push(`<div class="absolute inset-0 flex items-center justify-center z-10" style="background-color: var(--bg-secondary);"><div class="flex flex-col items-center gap-4 text-center px-4">`);
			Calendar($$renderer, { size: 48, class: 'text-yellow-500' });
			$$renderer.push(`<!----> <div><p class="text-lg font-semibold" style="color: var(--text-primary);">Not Yet Available</p> <p class="text-sm mt-1" style="color: var(--text-secondary);">This movie hasn't been released yet. Streaming will be available after release on ${escape_html(new Date(releaseDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }))}.</p></div></div></div>`);
		} else {
			$$renderer.push('<!--[!-->');
		...
		bind_props($$props, { tmdbId, imdbId, type, season, episode, releaseDate });
	});
}
