...
function MovieCard($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<!----> <span style="color: var(--text-secondary);">${escape_html(item.vote_average.toFixed(1))}</span></div></div> `);

		if (item.type === 'series' && (item.number_of_seasons || item.number_of_episodes)) {
			$$renderer.push('<!--[-->');
			$$renderer.push(`<div class="flex items-center gap-2 mt-1.5 text-xs" style="color: var(--text-secondary);">`);

			if (item.number_of_seasons) {
				$$renderer.push('<!--[-->');
				$$renderer.push(`<span class="series-stat svelte-156698q">${escape_html(item.number_of_seasons)} Season${escape_html(item.number_of_seasons !== 1 ? 's' : '')}</span>`);
			} else {
				$$renderer.push('<!--[!-->');
			}
			...
			if (item.number_of_seasons && item.number_of_episodes) {
				$$renderer.push('<!--[-->');
				$$renderer.push(`<span class="opacity-40">·</span>`);
			} else {
				$$renderer.push('<!--[!-->');
			}
			...
			if (item.number_of_episodes) {
				$$renderer.push('<!--[-->');
				$$renderer.push(`<span class="series-stat svelte-156698q">${escape_html(item.number_of_episodes)} Eps</span>`);
			} else {
				$$renderer.push('<!--[!-->');
			}
		...
		$$renderer.push(`<!--]--></div> <div class="absolute inset-0 rounded-xl border-2 border-transparent group-hover:border-amber-400/50 transition-all duration-300 pointer-events-none"></div></a>`);
